...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "runm_{$_args['post_type']}", 'runm' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "runm_{$_args['post_type']}", $posts, 'runm', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'runm' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'runm_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /run-manager directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with run-manager_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'runm_template_overwrite_dir', get_stylesheet_directory() . '/run-manager/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( RUN_MANAGER ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'run-manager' );
		}
	}
}